...
interface DuelContextType {
  ...
  submitSolution: (code: string, language: SupportedLanguage) => Promise<SubmissionResponse | null>;
  runTests: (code: string, language: SupportedLanguage) => Promise<SubmissionResponse | null>;
  isConnected: boolean;
  opponentCode: string;
  ...
  setSubmissionResult: (result: SubmissionResponse | null) => void;
}
...
export const DuelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setInitialLanguageFromDuel = useCallback(async (duelData: DuelResponse) => {
    if (duelData.problem && !currentLanguage) {
  ...
  const connect = useCallback((id: string) => {
    ...
    const socket = new WebSocket(`${BASE_WS_URL}/${id}?user_id=${user.id}`);
  ...
  const submitSolution = useCallback(async (code: string, language: SupportedLanguage): Promise<SubmissionResponse | null> => {
    if (!duel?.id || !user?.id) {
      addToast({ type: 'error', title: 'Error', message: 'Duel not active or user not logged in.' });
      return null;
    }
    try {
      const response = await duelsApiService.submitSolution(duel.id, { player_id: user.id, code, language: language.id });
      // No direct setting of submissionResult here, let WS message handle it
      return response;
    } catch (err: any) {
  ...
  const runTests = useCallback(async (code: string, language: SupportedLanguage): Promise<SubmissionResponse | null> => {
    if (!duel?.id) {
      addToast({ type: 'error', title: 'Error', message: 'Duel not active.' });
      return null;
    }
    try {
      const response = await duelsApiService.testCode(duel.id, { code, language: language.id });
      // No direct setting of submissionResult here, let WS message handle it
      return response;
    } catch (err: any) {
